...
term_freq = open("radlex_term_Freq_ddx_prady.csv").readlines()
# print(term_freq)
...
for nodeName, nodeID, parentID in rawTreeData:
    if not nodeID in setParents:
        ...
        if len(rawPath) > longestPath:
            ...
            # print("Longest path : "+unlist([dictIDToName[i]+" -> " for i in rawPath])+" \
            #     and its length : "+str(longestPath))
...
ofile = open("radtree_prady"+cat+"_rsna.csv", "w")
